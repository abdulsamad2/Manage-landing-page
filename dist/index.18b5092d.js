...
})({"3q8dD":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "63d81c4218b5092d";
...
const goToSlide = function(cs) {
    slide.forEach((slide1, index)=>{
        const xValue = (index - cs) * 100;
        slide1.style.transform = `translateX(${xValue}%)`;
    });
};
const activateDot = function(slde) {
    document.querySelectorAll(".dots-list").forEach((dot)=>{
        dot.classList.remove("bg-BrightRed");
        document.querySelector(`.dots-list[data-slide="${slde}"]`).classList.add("bg-BrightRed");
    });
};
...
const generateDots = function() {
    slide.forEach((_, i)=>{
        dotsContainer.insertAdjacentHTML("beforeend", `<li class=" bg-BrightRed w-3 h-3 rounded-full dots-list border-2 border-BrightRed" data-slide="${i}"></li>`);
    });
};
generateDots();
dotsContainer.addEventListener("click", function(e) {
    if (e.target.classList.contains("dots-list")) {
        const { slide: slide2  } = e.target.dataset;
        goToSlide(slide2);
        activateDot(slide2);
    }
});
...
//# sourceMappingURL=index.18b5092d.js.map
